...
void triggerSounds(void);
void addSoundFX(void);
void doNotes(void);
void doMoves(void);
...
int SoundPitchQueue[SOUND_QUEUE_SIZE];
int SoundLengthQueue[SOUND_QUEUE_SIZE];
int SoundDurationCounter=0;
int SoundQueueIndex=0;
int SoundInsertIndex=0;
...
int main()
{
  ...
  //ADCSRA=0xF0; //Enable, free running, high prescaler.
...
void addSoundFX()
{
  if(SoundQueueIndex == SoundInsertIndex)
  {
    SoundPitchQueue[0]=N_C0;
    SoundPitchQueue[1]=N_E0;
    SoundPitchQueue[2]=N_G0;
    SoundPitchQueue[3]=N_C1;
    SoundPitchQueue[4]=0;

    SoundLengthQueue[0]=250;
    SoundLengthQueue[1]=250;
    SoundLengthQueue[2]=250;
    SoundLengthQueue[3]=250;
    SoundLengthQueue[4]=2000;
    
    SoundInsertIndex = (SoundQueueIndex+5)%SOUND_QUEUE_SIZE;
  }
}
...
void doNotes(void)
{
  ...
  // To revert, delete from here...
  if(SoundQueueIndex != SoundInsertIndex)
  {
    SoundDurationCounter=SoundDurationCounter-1;
    if (SoundDurationCounter<1)
    {
      SoundQueueIndex=(SoundQueueIndex+1)%SOUND_QUEUE_SIZE; 
      if (SoundQueueIndex == SoundInsertIndex)
	return;
      if (SoundPitchQueue[SoundQueueIndex]==0)
        TCCR1B=0x00;
      else
      {
        OCR1A=SoundPitchQueue[SoundQueueIndex];
        TCCR1B=0x09;
      }
    }
  }
